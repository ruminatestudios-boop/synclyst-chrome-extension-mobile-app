...
type PageProps = {
  searchParams: Promise<{
    ...
    after_sign_up_url?: string | string[];
  }>;
};
...
export default async function SignUpPage({ searchParams }: PageProps) {
  ...
  const redirect = safeRedirectPath(
    firstString(params.redirect_url) ?? firstString(params.after_sign_up_url)
  );
  const signInUrl = `/sign-in?redirect_url=${encodeURIComponent(redirect)}&after_sign_in_url=${encodeURIComponent(redirect)}`;
